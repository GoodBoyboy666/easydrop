...
type userAdminURIRequest struct {
	ID uint `uri:"id" binding:"required,gt=0"`
}

type userAdminListQueryRequest struct {
...
func (h *UserAdminHandler) List(c *gin.Context) {
	...
	var req userAdminListQueryRequest
	if err := c.ShouldBindQuery(&req); err != nil {
		c.JSON(http.StatusBadRequest, MessageResponse{Message: "查询参数不合法"})
		return
	}

	result, err := h.userService.List(c.Request.Context(), dto.UserListInput{
...
// @Failure 401 {object} MessageResponse "未登录或登录失效"
// @Failure 403 {object} MessageResponse "无管理员权限"
// @Failure 409 {object} MessageResponse "用户名或邮箱冲突"
// @Failure 500 {object} MessageResponse "服务内部错误"
...
func (h *UserAdminHandler) Update(c *gin.Context) {
	...
	var uriReq userAdminURIRequest
...
// @Failure 400 {object} MessageResponse "参数校验失败"
// @Failure 401 {object} MessageResponse "未登录或登录失效"
// @Failure 403 {object} MessageResponse "无管理员权限或配额不足"
// @Failure 500 {object} MessageResponse "服务内部错误"
